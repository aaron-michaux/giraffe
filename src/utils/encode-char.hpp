...
{
   for(auto x : val) encode_char(ss, x);
   return ss;
}
...
inline auto decode_string(auto& s, auto begin, auto end) noexcept
{
   ...
   auto decode_hexidecimal_sequence = [&end](auto& ii) -> char {
      ...
      while(ii != end && std::isxdigit(*ii)) val = (val * 16) + convert_hex_digit(*ii++);
      return char(val);
   };
   ...
   for(auto ii = begin; ii != end; ++ii) {
      ...
      case '5': [[fallthrough]];
      case '6': [[fallthrough]];
      case '7': s.push_back(decode_octal_sequence(ii)); break;

      case 'x':
         // {HEXD}+
         if(std::next(ii) == end) return ii; // ERROR!
         ++ii;
         if(!std::isxdigit(*ii)) return ii; // ERROR
         s.push_back(decode_hexidecimal_sequence(ii));
         break;

         // We don't do unicode (to UTF8??)

      default: return ii; // ERROR!
      }
   }
